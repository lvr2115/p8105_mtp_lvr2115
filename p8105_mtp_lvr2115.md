...
The Excel file ‘USPS Change of Address NYC’ contains Change of Address
(COAs) forms received by USPS in NYC between 2018 and 2022. They include
the total number of COAs to and from each NYC zip code for each month of
...
`zipcodes` datafile is imported to clarify where borough and county
names may differ. A new variable `borough` is created in this dataset to
...
This table shows the five lowest values of average change in COAs. The
zipcodes are all in Gramercy Park/Murray Hill and the Lower East Side.
These experienced the highest number of move-outs relative to move-ins.
These are all at the onset of the COVID-19 pandemic between May-July
2020.
