...
# Introduction
The Excel file 'USPS Change of Address NYC' contains Change of Address (COAs) forms received by USPS in NYC between 2018 and 2022. They include the total number of COAs to and from each NYC zip code for each month of those 5 years. The goal of this report is to import and tidy the data, and then complete some exploratory analysis and visualizations for it. 

## Problem-1

`zipcodes` datafile is imported to clarify where borough and county names may differ. A new variable `borough` is created in this dataset to accurately reflect the borough each zipcode is located in. 
...
This table shows the five lowest values of average change in COAs. The zipcodes are all in Gramercy Park/Murray Hill and the Lower East Side. These experienced the highest number of move-outs relative to move-ins. These are all at the onset of the COVID-19 pandemic between May-July 2020.  
